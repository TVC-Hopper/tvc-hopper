...
typedef struct Bno085InitParams_s {
    ...
    uint8_t (*onWrite)(uint8_t address, uint8_t subaddress, uint8_t *buffer, uint32_t len);
    uint8_t (*onRead)(uint8_t address, uint8_t subaddress, uint8_t *buffer, uint32_t len);
    uint32_t (*getTime_us)();
} Bno085InitParams_t;
