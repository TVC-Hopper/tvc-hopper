...
r = 0.0128;           % m 
    ...
    dwx = (l*(CL*1*a1*(Kt1*wt^2 + Kt2*wt) + CL*1*a3*(Kt1*wt^2 + Kt2*wt)) + Jy*wy*wz - Jz*wy*wz)/Jx;
    dwy = -(l*(CL*1*a2*(Kt1*wt^2 + Kt2*wt) + CL*1*a4*(Kt1*wt^2 + Kt2*wt)) + Jx*wx*wz - Jz*wx*wz)/Jy;
    dwz = (r*(CL*1*a1*(Kt1*wt^2 + Kt2*wt) - CL*1*a2*(Kt1*wt^2 + Kt2*wt) - CL*1*a3*(Kt1*wt^2 + Kt2*wt) + CL*1*a4*(Kt1*wt^2 + Kt2*wt)) + Jx*wx*wy - Jy*wx*wy)/Jz;
%     dpx = vx;
%     dpy = vy;
%     dpz = vz;
...
%     dvz = (cos(p)*cos(q)*(Kt*wt^2 - CD*Kt*wt^2))/m - (sin(q)*(CL*Kt*a2*wt^2 + CL*Kt*a4*wt^2))/m - g*m + (cos(q)*sin(p)*(CL*Kt*a1*wt^2 + CL*Kt*a3*wt^2))/m;
    ...
    dvx = (g*m*sin(q) + CL*1*a2*(Kt1*wt^2 + Kt2*wt) + CL*1*a4*(Kt1*wt^2 + Kt2*wt))/m;
    dvy = (CL*1*a1*(Kt1*wt^2 + Kt2*wt) + CL*1*a3*(Kt1*wt^2 + Kt2*wt) - g*m*cos(q)*sin(p))/m;
    dvz = -(CD*1*(Kt1*wt^2 + Kt2*wt) - 1*(Kt1*wt^2 + Kt2*wt) + g*m*cos(p)*cos(q))/m;
