    ...
    methods
        ...
        function rx(obj, propIdx)
            ...
            for i = 1:obj.propertyStateArray(propIdx).property.numElts
                obj.propertyStateArray(propIdx).uiEditChildren(i).Value = double(obj.dataBuf(i));
            end
        end

        function tx(obj, propIdx)
            for i = 1:obj.propertyStateArray(propIdx).property.numElts
                obj.dataBuf(i) = obj.propertyStateArray(propIdx).property.castFunc(obj.propertyStateArray(propIdx).uiEditChildren(i).Value);
            end
