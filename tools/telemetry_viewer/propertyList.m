    ...
    methods
        function pl = propertyList()
            ...
            pl.props = [propertyEntry('LAUNCH HOPPER',         'uint8',   @uint8,   0,   1,  'W', 'C', false, @pl.dummy,                  [0], [""]),...
                        ...
                        propertyEntry('Z LIMIT SPINUP',       'single',  @single,  55,   4, 'RW', 'V', false, @pl.dummy,                  [1], ["z error spinup limit"]), ...
                        propertyEntry('Z LIMIT NORMAL',       'single',  @single,  56,   4, 'RW', 'V', false, @pl.dummy,                  [1], ["z error normal limit"]), ...
                        propertyEntry('MAX Z INT',            'single',  @single,  57,   4, 'RW', 'V', false, @pl.dummy,                  [1], ["max zerror integral"]), ...
                        propertyEntry('MAX ESC OUTPUT',       'single',  @single,  58,   4, 'RW', 'V', false, @pl.dummy,                  [1], ["max esc pulse width"]), ...
                        propertyEntry('K MATRIX',             'single',  @single,  54, 180, 'RW', 'M',  true, @pl.processKmatrix, [9 9 9 9 9], ["roll", "pitch", "yaw", "angVelx", "angVely", "angVelz", "zpos", "zvel", "integral"])];
        end
